...
for f in glob.iglob("./TA.I47A.*.recf.sac"):
    st += read(f)
    dummy_names = np.append(dummy_names, f)

for tr in st:
    dummy = np.append(dummy,tr.stats.sac.baz)
    fff = np.append(fff, tr)

idx = np.argsort(dummy)

st1 = Stream()

for i in dummy_names[idx]:
    file = i
    xx = '{}'.format(file)
...
zz = 180 - (((len(idx)) / 2) * 10)
if zz < 0:
    print("ERROR: OFFSET IS NEGATIVE!")
...
def _fig2():
    ax2.set_ylim([25, 0])
    ax2.set_yticks([])
    ax2.grid(visible=True, which='major', axis='x', linewidth=0.5)
